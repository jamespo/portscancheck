...
def main():
    parser = argparse.ArgumentParser(
        description="Nagios plugin to check for changes in open ports using nmap."
    )
    parser.add_argument("hostname", help="Server hostname or IP to scan")
    parser.add_argument(
        "directory",
        nargs="?",
        default="/etc/nagios/portscancheck",
        help="Directory to store baseline and results (default: /etc/nagios/portscancheck)",
    )
    parser.add_argument(
        "-6", dest="ipv6", action="store_const", const="-6", help="Use IPv6"
    )
